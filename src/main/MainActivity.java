...
public class MainActivity extends Activity {
    ...
    private long copyBigRange(OutputStream out, InputStream in, long currentPos, int index) throws Exception {
        BigEntry e=currentBigIndex.get(index);
        if(e.offset < currentPos){
            try { in.close(); } catch(Exception ignored) {}
            in=getContentResolver().openInputStream(currentBigUri);
            if(in==null) throw new Exception("Не удалось открыть исходный BIG");
            currentPos=0;
        }
        if(e.offset > currentPos) currentPos += skipFully(in,e.offset-currentPos);
        ...
        while(left>0){
            ...
            if(n==0) continue;
            out.write(buf,0,n); left-=n; currentPos+=n;
        }
        return currentPos;
    }
    ...
    private void saveBigStreaming(Uri outUri, org.json.JSONArray arr) throws Exception {
        ...
        try {
            ...
            try {
                ...
                InputStream source=null; long sourcePos=0;
                try {
                    source=getContentResolver().openInputStream(currentBigUri);
                    if(source==null) throw new Exception("Не удалось открыть исходный BIG");
                    for(int i=0;i<count;i++){
                        ...
                        if(o.optBoolean("modified",false) || o.optInt("index",-1)<0){
                            ...
                            if(d.length!=size) throw new Exception("Размер изменённого файла не совпадает");
                            out.write(d);
                        } else {
                            sourcePos=copyBigRange(out,source,sourcePos,o.getInt("index"));
                        }
                    }
