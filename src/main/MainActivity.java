...
public class MainActivity extends Activity {
    ...
    private Uri pendingBigSourceUri;
    ...
    private long currentBigTotal = 0;
    private long currentBigDataStart = 16;
    ...
    private List<BigEntry> readBigIndex(Uri uri) throws Exception {
        ...
        try {
            ...
            currentBigTotal=actualTotal;
            currentBigDataStart=dataStart;
    ...
    private InputStream openBigRange(Uri sourceUri, long offset) throws Exception {
    ...
    private byte[] readBigEntryBytes(int index) throws Exception {
        ...
        InputStream in=openBigRange(currentBigUri,e.offset);
        try { return readExactly(in,(int)e.size); } finally { in.close(); }
    }

    private void copyBigEntry(OutputStream out, int index, Uri sourceUri) throws Exception {
        if(index<0 || index>=currentBigIndex.size()) throw new Exception("Неверный индекс BIG");
        BigEntry e=currentBigIndex.get(index);
        InputStream in=openBigRange(sourceUri,e.offset);
        try {
            ...
            while(left>0){
                ...
                if(n<0) throw new Exception("Неожиданный конец BIG: "+e.name);
                if(n==0) continue;
                out.write(buf,0,n);
                left-=n;
            }
    ...
    private boolean manifestMatchesOriginal(org.json.JSONArray arr) throws Exception {
        if (currentBigUri == null || arr.length() != currentBigIndex.size()) return false;
        for (int i = 0; i < arr.length(); i++) {
            org.json.JSONObject o = arr.getJSONObject(i);
            if (o.optBoolean("modified", false)) return false;
            int oldIndex = o.optInt("index", -1);
            if (oldIndex != i) return false;
            BigEntry old = currentBigIndex.get(i);
            if (!old.name.equals(o.getString("name"))) return false;
            if (old.size != o.getLong("size")) return false;
        }
        return true;
    }

    private void copyUriToFile(Uri sourceUri, File dst) throws Exception {
        InputStream in = getContentResolver().openInputStream(sourceUri);
        if (in == null) throw new Exception("Не удалось открыть исходный BIG");
        try {
    ...
    private void copyTempToUri(File tmp, Uri outUri) throws Exception {
        ContentResolver cr = getContentResolver();
    ...
    private void saveBigStreaming(final Uri outUri, final org.json.JSONArray arr) throws Exception {
        saveBigStreaming(outUri, arr, currentBigUri);
    }

    private void saveBigStreaming(final Uri outUri, final org.json.JSONArray arr, final Uri sourceUri) throws Exception {
        // Do all heavy I/O off the WebView/UI thread. A 1.4+ GB BIG can take a
        // long time to rebuild/copy and must never freeze the menu or Activity.
        new Thread(new Runnable() {
            @Override public void run() {
                File tmp = null;
                try {
                    tmp = File.createTempFile("genmod_", ".big.tmp", getCacheDir());

                    // If absolutely nothing in the archive changed, preserve the
                    // original BIG byte-for-byte. This keeps its exact original
                    // size, padding and layout and is the safest possible save.
                    if (sourceUri != null && manifestMatchesOriginal(arr)) {
                        copyUriToFile(sourceUri, tmp);
                    } else {
                        buildBigTemp(tmp, arr, sourceUri);
                    }

                    copyTempToUri(tmp, outUri);
                    final String msg = "BIG сохранён";
                    runOnUiThread(new Runnable() { @Override public void run() {
                        MainActivity.this.run("window.onSaveResult&&window.onSaveResult(true," + jsString(msg) + ")");
                    }});
                } catch (final Exception e) {
                    final String msg = "Ошибка сохранения: " + String.valueOf(e.getMessage());
                    runOnUiThread(new Runnable() { @Override public void run() {
                        MainActivity.this.run("window.onSaveResult&&window.onSaveResult(false," + jsString(msg) + ")");
                    }});
                } finally {
    ...
    private void saveBigStreamingSync(Uri outUri, org.json.JSONArray arr, Uri sourceUri) throws Exception {
        File tmp = File.createTempFile("genmod_", ".big.tmp", getCacheDir());
        try {
            if (sourceUri != null && manifestMatchesOriginal(arr)) copyUriToFile(sourceUri, tmp);
            else buildBigTemp(tmp, arr, sourceUri);
            copyTempToUri(tmp, outUri);
        } finally { if (tmp.exists()) tmp.delete(); }
    }

    private void buildBigTemp(File tmp, org.json.JSONArray arr, Uri sourceUri) throws Exception {
        int count = arr.length();
        long idx = 16;
        for (int i = 0; i < count; i++) {
            byte[] name = arr.getJSONObject(i).getString("name")
                    .replace('/', '\\').getBytes("UTF-8");
            idx += 8 + name.length + 1;
        }
        if (idx > 0xffffffffL) throw new Exception("Индекс BIG больше 4 ГБ");
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (size < 0 || size > 0xffffffffL) throw new Exception("Недопустимый размер: " + o.getString("name"));
            offsets[i] = total;
            total += size;
        ...
        try {
            byte[] h = new byte[16];
            String magic = (currentBigMagic == null || currentBigMagic.length() != 4) ? "BIGF" : currentBigMagic;
            System.arraycopy(magic.getBytes("US-ASCII"), 0, h, 0, 4);
            putU32LE(h, 4, total);
            putU32BE(h, 8, count);
            putU32BE(h, 12, idx);
            out.write(h);

            // Write the complete index first.
            for (int i = 0; i < count; i++) {
                org.json.JSONObject o = arr.getJSONObject(i);
                byte[] name = o.getString("name").replace('/', '\\').getBytes("UTF-8");
                putU32BE(h, 0, offsets[i]);
                putU32BE(h, 4, o.getLong("size"));
                out.write(h, 0, 8);
                out.write(name);
                out.write(0);
            }

            // Then write payloads sequentially. Unmodified entries are copied
            // directly from their original offsets; modified/new entries come
            // from the manifest base64 data.
            for (int i = 0; i < count; i++) {
                org.json.JSONObject o = arr.getJSONObject(i);
                long size = o.getLong("size");
                boolean modified = o.optBoolean("modified", false);
                int oldIndex = o.optInt("index", -1);
                if (modified || oldIndex < 0 || sourceUri == null) {
                    byte[] d = b64(o.optString("b64", ""));
                    if (d.length != size) {
                        throw new Exception("Размер изменённого файла не совпадает: " + o.getString("name") +
                                " (ожидалось " + size + ", получено " + d.length + ")");
                    }
                    out.write(d);
                } else {
                    copyBigEntry(out, oldIndex, sourceUri);
                }
            }
            out.flush();
        } finally { out.close(); }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode != RESULT_OK || data == null) {
            if (requestCode == REQ_CREATE_BIG) pendingBigSourceUri=null;
            ...
            switch (requestCode) {
                ...
                case REQ_CREATE_BIG: {
                    ...
                    if(pendingBigManifest!=null){
                        Uri sourceUri=pendingBigSourceUri;
                        String manifest=pendingBigManifest;
                        pendingBigManifest=null; pendingBigSaveAsName=null; pendingBigSourceUri=null;
                        final Uri destUri=uri;
                        final Uri srcUri=sourceUri;
                        final org.json.JSONArray saveArr=new org.json.JSONArray(manifest);
                        new Thread(new Runnable(){ @Override public void run(){
                            try {
                                saveBigStreamingSync(destUri, saveArr, srcUri);
                                currentBigUri=destUri;
                                runOnUiThread(new Runnable(){ @Override public void run(){
                                    MainActivity.this.run("window.onSaveResult&&window.onSaveResult(true,"+jsString("BIG сохранён: "+nameFromUri(destUri))+")");
                                }});
                            } catch(final Exception e){
                                runOnUiThread(new Runnable(){ @Override public void run(){
                                    MainActivity.this.run("window.onSaveResult&&window.onSaveResult(false,"+jsString("Ошибка сохранения: "+e.getMessage())+")");
                                }});
                            }
                        }} , "BIG-SaveAs").start();
        ...
        public void saveBigStreaming(String manifestJson, boolean saveAs, String suggestedName) {
            try {
                ...
                if(saveAs || currentBigUri==null){
                    pendingBigManifest=manifestJson; pendingBigSaveAsName=suggestedName; pendingBigSourceUri=currentBigUri;
