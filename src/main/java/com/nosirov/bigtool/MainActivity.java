...
public class MainActivity extends Activity {
    ...
    private void saveBigStreaming(Uri outUri, org.json.JSONArray arr) throws Exception {
        Uri sourceUri=currentBigUri;
        saveBigStreaming(outUri,arr,sourceUri);
    }

    private void saveBigStreaming(Uri outUri, org.json.JSONArray arr, Uri sourceUri) throws Exception {
        int count=arr.length();
        long idx=16;
        for(int i=0;i<count;i++){
            byte[] name=arr.getJSONObject(i).getString("name").replace('/','\\').getBytes("UTF-8");
            idx += 8 + name.length + 1;
        }
        if(idx>0xffffffffL) throw new Exception("Индекс BIG больше 4 ГБ");

        // Preserve the original layout/padding whenever possible. This is important for
        // Generals BIGs that contain alignment gaps between payloads. If no entry size
        // changed, the resulting archive keeps the original physical size.
        long[] outOffsets=new long[count];
        long pos=idx;
        long originalPrevEnd=currentBigDataStart;
        long sizeDelta=0;
        for(int i=0;i<count;i++){
            ...
            long gap=0;
            if(oldIndex>=0 && oldIndex<currentBigIndex.size()){
                BigEntry old=currentBigIndex.get(oldIndex);
                gap=Math.max(0, old.offset-originalPrevEnd);
                originalPrevEnd=old.offset+old.size;
                sizeDelta += size-old.size;
            }
            pos += gap;
            outOffsets[i]=pos;
            pos += size;
        }
        long originalTrailing=0;
        if(currentBigTotal>0 && originalPrevEnd<=currentBigTotal)
            originalTrailing=currentBigTotal-originalPrevEnd;
        long total=pos+originalTrailing;
        ...
        try {
            ...
            try {
                ...
                for(int i=0;i<count;i++){
                    org.json.JSONObject o=arr.getJSONObject(i);
                    long gap=outOffsets[i]-written;
                    while(gap>0){ int n=(int)Math.min(zeros.length,gap); out.write(zeros,0,n); gap-=n; }
                    long size=o.getLong("size");
                    if(o.optBoolean("modified",false) || o.optInt("index",-1)<0){
                        byte[] d=b64(o.optString("b64",""));
                        if(d.length!=size) throw new Exception("Размер изменённого файла не совпадает: "+o.getString("name"));
                        out.write(d);
                    } else {
                        copyBigEntry(out,o.getInt("index"),sourceUri);
                    }
                    written=outOffsets[i]+size;
                }
                while(written<total){
                    int n=(int)Math.min(zeros.length,total-written); out.write(zeros,0,n); written+=n;
                }
                out.flush();
            } finally { out.close(); }
            ...
            switch (requestCode) {
                ...
                case REQ_CREATE_BIG: {
                    ...
                    if(pendingBigManifest!=null){
                        ...
                        saveBigStreaming(uri,new org.json.JSONArray(pendingBigManifest),sourceUri);
