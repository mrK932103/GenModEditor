...
public class MainActivity extends Activity {
    ...
    private List<BigEntry> readBigIndex(Uri uri) throws Exception {
        ...
        try {
            ...
            long total=readU32BE(h,4)&0xffffffffL;
            long count=readU32BE(h,8)&0xffffffffL;
            long dataStart=readU32BE(h,12)&0xffffffffL;
            if(count>200000 || dataStart<16 || dataStart>total) throw new Exception("Повреждённый индекс BIG");
            for(int i=0;i<count;i++){
                ...
                if(off+len>total) throw new Exception("Данные файла выходят за пределы архива");
    ...
    private void saveBigStreaming(Uri outUri, org.json.JSONArray arr) throws Exception {
        ...
        try {
            ...
            try {
                ...
                putU32BE(h,4,total); putU32BE(h,8,count); putU32BE(h,12,idx); out.write(h);
                long off=idx;
                for(int i=0;i<count;i++){
